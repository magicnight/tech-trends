...
tech-trends 通过环境变量进行配置，支持 `.env` 文件自动加载。所有变量均有合理的默认值，最少只需配置 LLM API Key 即可运行。
...
> `sync`、`backtest`、`status`、`reindex` 命令不需要 LLM API Key。`calibrate` 也不需要。

## .env 文件

tech-trends 启动时自动加载项目目录下的 `.env` 文件（通过 `dotenvy`），无需手动 source。
...
> 更换 Embedding 模型后，需要运行 `tech-trends reindex` 重建 Qdrant 向量索引。
